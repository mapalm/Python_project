...
available_data_types = ['all information (all)', 'accession numbers (accessions)', 'resistance patterns (resistances)', 'positions in microtitre plates (positions)']
...
if len(query) > 0 and (query == 'resistances' or query == 'Resistances'):
    ...
    print('Please enter a correct type of data. The available types of data are %s, %s, %s and %s.' % (available_data_types[0], available_data_types[1], available_data_types[2], available_data_types[3]))

# Creates a folder named for the specific input file if it does not exist
